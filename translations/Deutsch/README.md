...
SA-MP Injector ist ein vielseitiges Tool, das für **SA-MP (San Andreas Multiplayer)** und **OMP (Open Multiplayer)** entwickelt wurde und sowohl als **Kommandozeilen-Executable** als auch als **Bibliothek für verschiedene Programmiersprachen** funktioniert. Seine Hauptfunktion besteht darin, `samp.dll` (für **SA-MP**) oder sowohl `samp.dll` als auch `omp-client.dll` (für **OMP**) in den Prozess von **GTA:SA** zu injizieren, um das Spiel mit benutzerdefinierten Parametern für eine direkte Verbindung zu einem Server zu starten.
...
3. **Dynamische DLL-Injektion:**
    ...
    Die Injektion wird durch den entfernten Aufruf der Funktion `LoadLibraryA` erleichtert, wodurch GTA:SA die Multiplayer-Module lädt und die Verbindung initialisiert.
...
| Pascal/Delphi | **SA-MP Injector P/D**     | (Demnächst)                         |
| PowerShell    | **SA-MP Injector PS**      | (Demnächst)                         |
